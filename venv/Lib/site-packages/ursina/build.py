...
if build_engine:
    ...
    [copy(str(f), str(python_dest / f.name))
     for f in python_folder.iterdir() if f.is_file()]

    # def copy_python_lib():
    print('copying python Lib files')
    [copy(str(f), str(python_lib_dest / f.name))
     for f in Path(python_folder / 'Lib').iterdir() if f.is_file()]
    ...
    for f in (python_folder / 'Lib/site-packages/panda3d/').iterdir():
        if f.name in (
            ...
            continue
        print('copying:', f, '-->', str(python_dest /
              'Lib/site-packages/panda3d/' / f.name))
        if f.is_file():
            copy(str(f), str(python_dest / 'Lib/site-packages/panda3d/' / f.name))
        else:
            (python_dest / 'Lib/site-packages/panda3d/' /
...
if build_game:
    ...
    if compile_to_pyc:
        ...
        for f in project_folder.glob('**/*.py'):
            ...
            print('compiling:', f, src_dest / str(f)
                  [len(str(project_folder))+1:])
            py_compile.compile(
                f, src_dest / (str(f)[len(str(project_folder))+1:]+'c'))
    ...
    for f in project_folder.iterdir():
        ...
        if name in ignore:
            ...
            copytree(project_folder / f, dest,
                     ignore_filetypes=ignore_patterns)
        elif f.is_file():
